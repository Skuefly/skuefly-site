...
- **Model policy — Josh never picks models, and a premium session is not a file processor.** Main sessions run the best available tier (Fable 5 / Opus 5). Before large mechanical jobs — repo scans, audits, file comparisons, migrations, formatting sweeps, repetitive edits, data extraction — delegate to workers instead. Workers use the cheapest capable model (Haiku/Sonnet). Primary sessions handle architecture, ambiguity, review, and final approval. Silent technical decision (ratified 2026-07-29).
