...
- **Model policy — Josh never picks models, and a premium session is not a file processor.** Main sessions run **Opus 5** ($5/$25 per 1M). **Fable 5 is the TOP tier, not a cheap one — $10/$50, twice Opus** — reserved for the hardest long-horizon work and never for routine jobs. Before large mechanical jobs — repo scans, audits, file comparisons, migrations, formatting sweeps, repetitive edits, data extraction — delegate to workers and **name the model explicitly**: Haiku ($1/$5) for routine checks and mechanical execution, Sonnet ($3/$15) for scoped implementation and analysis. A subagent inherits the main-loop model unless one is passed, so an Opus session does NOT delegate cheaply by default. Primary sessions handle architecture, ambiguity, review, and final approval. Full tier table and effort guidance: `rules/continuation.md`. (Ratified 2026-07-29; Fable tier corrected against live pricing 2026-07-31 — it had been grouped with Opus as one "best available tier", which would have put the most expensive model on routine work.)
...
- **Recurring work is stateless (ratified 2026-07-28).** No schedule ever fires into a long-lived session; loops = fresh session + playbook file + capped rolling brief in the repo; cheap "any work?" check before loading anything heavy; workers default to Sonnet. A session past ~200k context hands its recurring duties off.
